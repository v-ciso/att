...
interface ReportData { batches: Batch[]; profiles: Profile[]; roster: RosterPerson[]; operatingStartDate: string | null }
...
function LiveImportReport() {
  ...
  async function mapPerson(summary: Summary, profileId?: string, rosterPerson?: RosterPerson) {
    setMappingId(summary.externalRepId); setError('');
    try {
      const response = await fetch('/api/dd-reports/profiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ externalRepId: summary.externalRepId, reportName: summary.reportName, profileId, displayName: rosterPerson?.name ?? summary.reportName, employeeCode: rosterPerson?.employeeCode }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error);
